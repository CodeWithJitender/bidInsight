...
//      const [paymentResult, setPaymentResult] = useState(status || 'unpaid');
...
//     // const status = params.get("status") || params.get("payment_status") || "unpaid";
...
//             { type: "link", text: "Retry Payment", url: "/pricing" }
//         ],
//         note: {
...
//     // NEW: Unpaid/Incomplete data for when user returns without completing payment
//     const unpaidData = {
//         image: "/payment-ussuccessfull.png",
//         title: "Payment Incomplete",
//         description: "You returned without completing the payment process.",
//         details: [
//             { 
//                 label: "Status", 
//                 value: "UNPAID"
...
//     return [successData, failedData, unpaidData];
// };
...
function PaymentStatusInner({ planPrice, status, transactionData }) {
    const [open, setOpen] = useState(false);
    const [paymentResult, setPaymentResult] = useState(status || 'unpaid'); // Changed default from 'success' to 'unpaid'
    const [paymentData, setPaymentData] = useState(transactionData);
    ...
    const getPopupContent = () => {
        ...
        if (paymentResult === 'success') {
            ...
            // Dynamic button - ONLY for successful payments show onboarding
            const successButtons = hasProfile
                ? [{ type: "link", text: "Go to Dashboard", url: "/dashboard" }]
                : [{ type: "link", text: "Complete your onboarding", url: "/geographic-coverage" }];
            ...
            return processData[1]; // Failed - Dashboard + Retry buttons
        } else if (paymentResult === 'unpaid' || paymentResult === 'incomplete') {
            return processData[2]; // Unpaid/Incomplete - Dashboard + Retry buttons
        }
        
        // Default fallback - Dashboard + Retry buttons
        return {
            image: "/payment-ussuccessfull.png",
            title: "Processing Payment",
            details: [],
...
export default function PaymentStatus() {
    ...
    // Extract route parameters - YEH LINE 38 KO YAHAN LANA THA
    const status = params.get("status") || params.get("payment_status") || "unpaid"; // FIXED: Moved here and changed default
    const planPrice = params.get("plan_price") || "29.99";
    const transactionId = params.get("transaction_id");
    const transactionDate = params.get("transaction_date");
    const amount = params.get("amount");

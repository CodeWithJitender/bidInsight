          ...
          <table className="min-w-full table-auto text-sm text-center">
            <thead className="sticky z-10 top-0 bg-white/5 backdrop-blur-sm">
              <tr className="text-white/80 text-xs border-b border-white/20">
                  ...
                  <div ref={dropdownRef} className="inline-block text-left">
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-1 rounded-full relative ${
                        restrictions.entityDropdown ? "opacity-60" : ""
                      }`}
                      title={
                ...
                <th
                  className={`px-4 py-4 font-inter text-lg ${
            ...
            <tbody>
              {data.length === 0 ? (
                ...
                data.map((bid, index) => {
                  ...
                  if (index < 5) {
                    console.log(
                      `Bid ${index}: shouldBlurThisBid=${shouldBlurThisBid}, blurConfig=`,
                      blurConfig
                    );
                  }
            ...
            data.map((bid, index) => {
              ...
              return (
                <div
                  key={bid.id}
                  onClick={() => handleRowClick(bid.id, index)}
                  className={`bg-white/5 backdrop-blur-sm rounded-xl border border-white/20 p-4 cursor-pointer hover:bg-white/10 transition ${
                    shouldBlurThisBid ? "opacity-75" : ""
                  }`}
                >
            ...
            data.map((bid, index) => {
              ...
              return (
                <div
                  key={bid.id}
                  onClick={() => handleRowClick(bid.id, index)}
                  className={`text-white border-b-[2px]  px-2 py-4 flex flex-wrap items-center justify-between gap-6 transition  cursor-pointer ${
                    shouldBlurThisBid ? "opacity-75" : ""
                  }`}
                >
                  ...
                  <div className="w-[70%] flex-1 min-w-[300px]">
                    ...
                    <div className="flex items-center gap-8 mt-3 text-xs ">
                      ...
                      <BlurWrapper
                        shouldBlur={shouldBlurColumn("closed_date", index)}
                      >
                        <div>
                          <p className="font-inter text-lg font-semibold text-[#DBDBDB]">Closed Date</p>
                          <p className="font-inter text-base ">
                            {formatDate(bid.closed_date)}
                          </p>
                        </div>
                      </BlurWrapper>
                    </div>
                  ...
                  <div className="w-[15%] flex flex-col justify-between items-end  min-w-[60px]" onClick={(e) => e.stopPropagation()}>
                    {/* Share */}
                    {restrictions?.share ? (
                      <div
                        className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center opacity-50"

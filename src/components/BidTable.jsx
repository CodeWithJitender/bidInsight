...
const BidTable = forwardRef(
  (
    ...
    const safeFollowLoading =
      followLoading instanceof Set ? followLoading : new Set();
    const safeFollowedBids =
      followedBids instanceof Set ? followedBids : new Set();
    ...
    const isStarterPlan =
      planData?.plan_code === "002" && planData?.name === "Starter";
    ...
    const handleFollowClick = (e, bidId) => {
      ...
      const isFollowed =
        followedBids && followedBids.has ? followedBids.has(bidId) : false;
    ...
    const getSortIcon = (field) => {
      ...
      const isCurrentField =
        currentSortField === field || currentSortField === `-${field}`;
      const isDescending = currentSortField === `-${field}`;
      if (!isCurrentField)
        return (
          <span className="ml-2">
            <i className="fas fa-sort text-white/50 text-xs"></i>
          </span>
        );
      return (
        <span className="ml-2">
          <i
            className={`fas ${isDescending ? "fa-sort-down" : "fa-sort-up"
              } text-white text-xs`}
          ></i>
        </span>
      );
    ...
    return (
      <>
            ...
            <table className="min-w-full table-auto text-sm text-center">
              <thead className="sticky z-10 top-0 bg-white/5 backdrop-blur-sm">
                <tr className="text-white/80 text-xs border-b border-white/20">
                      ...
                      <button
                        ...
                        title={
                          restrictions.entityDropdown
                            ? "Upgrade to filter by entity type"
                        ...
                        <div className="absolute mt-2 w-40 rounded-md bg-white text-black font-medium z-10">
                          {getEntityTypes().map((type) => (
                            <div
                              ...
                              className={`px-4 py-2 hover:bg-gray-100 cursor-pointer ${selectedEntity === type
                  ...
                  <th
                    className={`px-4 py-4 font-inter text-lg ${planInfo?.isFree && restrictions?.sorting
                      ? " opacity-60"
                      : "cursor-pointer"
                      }`}
                    onClick={(e) => handleHeaderClick("open_date", e)}
                    title={
                      planInfo?.isFree && restrictions?.sorting
                        ? "Upgrade to sort by open date"
                        : "Click to sort by open date"
                    }
                  >
                  ...
                  <th
                    ...
                    title={
                      planInfo?.isFree && restrictions?.sorting
                        ? "Upgrade to sort by closing date"
                  ...
                  <th
                    ...
                    title={
                      planInfo?.isFree && restrictions?.sorting
                        ? "Upgrade to sort by countdown"
                        : "Sort by countdown (closing date)"
                    }
                  >
                  ...
                  <th className="px-4 py-4 font-inter text-lg text-center">
                    Share
                  </th>
                  <th className="px-4 py-4 font-inter text-lg text-center">
                    Follow
                ...
                {data.length === 0 ? (
                  ...
                  data.map((bid, index) => {
                    ...
                    const isClosingToday =
                      closingDateObj.toDateString() === today.toDateString();
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-4 font-medium font-inter">
                          <BlurWrapper
                            shouldBlur={shouldBlurColumn("bid_name", index)}
                          >
                        ...
                        <td className="px-4 py-4 font-medium font-inter">
                          <BlurWrapper
                            shouldBlur={shouldBlurColumn("open_date", index)}
                          >
                        ...
                        <td className="px-4 py-4 font-medium font-inter">
                          <BlurWrapper
                            shouldBlur={shouldBlurColumn("closing_date", index)}
                          >
                        ...
                        >
                          <BlurWrapper
                            shouldBlur={shouldBlurColumn("countdown", index)}
                          >
                        ...
                        <td className="px-4 py-4 text-center flex justify-center items-center">
                          {restrictions?.share ? (
                            ...
                            <BlogShareButton
                              ...
                              onShare={() =>
                                console.log(`Shared bid: ${bid.bid_name}`)
                              }
                            />
                          )}
                        </td>
        ...
        {viewMode && (
          <div className="block bg-btn  text-white py-8  rounded-[21px] shadow-xl overflow-x-auto border-white border-2 border-solid space-y-3 my-[50px] px-4 max-h-screen overflow-y-auto">
            {data.length === 0 ? (
              <div className="text-center py-12 text-white/60">
                <div className="w-16 h-16 bg-white/10 rounded-full flex items-center justify-center mx-auto mb-4">
                  <i className="fas fa-inbox text-2xl"></i>
              ...
              data.map((bid, index) => {
                ...
                return (
                  <div
                    key={bid.id}
                    onClick={() => handleRowClick(bid.id, index)}
                    className={`text-white border-b-[2px]  px-2 py-4 flex flex-wrap items-center justify-between gap-6 transition  cursor-pointer ${shouldBlurThisBid ? "opacity-75" : ""
                      }`}
                  >
                    {/* Left Column: Entity Type + Status */}
                    <div className="w-[15%] flex flex-col gap-3 min-w-[160px]">
                      <div>
                        <p className="font-inter text-lg font-semibold text-[#DBDBDB]">Entity Type</p>
                        <BlurWrapper
                          shouldBlur={shouldBlurColumn("entity_type", index)}
                        >
                      ...
                      <div>
                        <p className="font-inter text-lg font-semibold text-[#DBDBDB]">Status</p>
                        <p className="font-inter text-lg ">
                          {statusLabel}
                        </p>
                      </div>
                    ...
                    {/* Middle Column: Bid Name + Dates */}
                    <div className="w-[70%] flex-1 min-w-[300px]">
                      <p className="font-inter text-lg font-semibold text-[#DBDBDB]">Bid Name</p>
                      <BlurWrapper
                        shouldBlur={shouldBlurColumn("bid_name", index)}
                      >
                      ...
                      <div className="flex items-center gap-8 mt-3 text-xs ">
                        ...
                        >
                          <div>
                            <p className="font-inter text-lg font-semibold text-[#DBDBDB]">Countdown</p>
                        ...
                        >
                          <div>
                            <p className="font-inter text-lg font-semibold text-[#DBDBDB]">Open Date</p>
                            <p className="font-inter text-base ">
                              {formatDate(bid.open_date)}
                            </p>
                          </div>
                        ...
                        >
                          <div>
                            <p className="font-inter text-lg font-semibold text-[#DBDBDB]">Closed Date</p>
                            <p className="font-inter text-base ">
                              {formatDate(bid.closing_date)}
                            </p>
                          </div>
                    ...
                    {/* Right Column: Share + Bookmark */}
                    <div className="w-[15%] flex flex-col justify-between items-end gap-7  min-w-[60px]" onClick={(e) => e.stopPropagation()}>
                      {/* Share */}
                      {restrictions?.share ? (
                        <div
                          className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center opacity-50"
                        ...
                        <>
                          {/* <p className="font-inter text-lg font-semibold m-0 text-[#DBDBDB]">Share</p> */}
                          <BlogShareButton
                            url={`${window.location.origin}/summary/${bid.id}`}
                            onShare={() =>
                              console.log(`Shared bid: ${bid.bid_name}`)
                      ...
                      {/* Bookmark */}
                      {/* Follow Button - COMPLETE IMPLEMENTATION */}
                      <div className="flex flex-col items-end gap-2">
                        {/* <p className="font-inter text-lg font-semibold m-0 text-[#DBDBDB]">Follow</p> */}
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            if (restrictions?.follow) {
                          ...
                          disabled={safeFollowLoading.has(bid.id)}
                          className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 ${restrictions?.follow
                            ? "opacity-50 bg-white/10 cursor-not-allowed"
                            : safeFollowLoading.has(bid.id)
                              ? "opacity-50 cursor-not-allowed"
                              : "hover:scale-110 cursor-pointer"
                            }`}
                          title={
                        ...
                        >
                          {restrictions?.follow ? (
                            ...
                            <i
                              className={`fas text-lg transition-colors ${safeFollowedBids.has(bid.id)
                                ? "fa-minus-circle"
                                : "fa-plus-circle "
                                }`}
                            />
